...
if ($_SESSION['user'] == "admin") { ?>
    ...
    <head>
        <style>
            .card {
                ...
                position: relative;
                width: 105%;
                /* or 100% if you want it flush edge‑to‑edge */
                margin-left: auto;
                margin-right: auto;
            }
        ...
        <style>
            ...
            /* Remove native icon (but keep picker functional) */
    ...
    <body>
        ...
        <div id="leaveModal" class="modal">
            <div class="modal-content">
                <span class="close" onclick="closeLeaveModal()"
                    style="position: absolute; top: 10px; right: 10px; font-size: 30px; cursor: pointer;">&times;</span>
                ...
                <form method="post" action="admin_attendance.php">
                    ...
                    <div class="date-wrapper">
                        ...
                        <span class="calendar-icon" onclick="document.getElementById('from_date').showPicker()">📅</span>
                    </div>

                    <label><b>To Date:</b></label>
                    <div class="date-wrapper">
                        <input type="date" id="to_date" name="to_date" required>
                        <span class="calendar-icon" onclick="document.getElementById('to_date').showPicker()">📅</span>
                    </div>

                    <!-- New Dropdown -->
                    <label><b>Leave Type:</b></label>
                    <select name="leave_type" id="leave_type" required style="width: 100%; padding: 6px 10px; border: 1px solid #ccc; border-radius: 4px; font-size: 14px; margin-bottom: 10px;">
                    ...
                    <div style="text-align: center;">
                        <button type="submit" name="submit_leave"
                            style="background-color: orange; color: white; padding: 5px 14px; border: none; border-radius: 5px; cursor: pointer;">
                            Submit Leave
                        </button>
                    </div>
                </form>
        ...
        if (isset($_POST['submit_leave'])) {
            ...
            if ($name) {
                // check if already leave applied between these dates
                $stmt = $conn->prepare("SELECT id FROM emp_attendance WHERE empcode = ? AND (date BETWEEN ? AND ?)");
                $stmt->bind_param("iss", $emp_id, $from_date, $to_date);
                $stmt->execute();
                $stmt->store_result();

                if ($stmt->num_rows > 0) {
                    echo "<script>showAlert('Leave already marked in this date range');</script>";
                } else {
                    $stmt->close();

                    // insert leave record
                    $stmt = $conn->prepare("INSERT INTO emp_attendance 
                (empcode, name, date, from_date, to_date, leave_type, check_in_time, check_out_time, status, work_hours, leave_reason) 
                VALUES (?, ?, CURDATE(), ?, ?, ?, '0', '0', 'Leave', '0', ?)");

                    $stmt->bind_param("isssss", $emp_id, $name, $from_date, $to_date, $leave_type, $leave_reason);
                    $stmt->execute();
                    $stmt->close();
